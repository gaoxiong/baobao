...
public abstract class Watson extends FragmentActivity
  ...
  private List<Fragment> createChildrenMenus(Menu menu, MenuInflater inflater,
      List<Fragment> fragments) {
    if (fragments == null || fragments.isEmpty()) {
      return Collections.EMPTY_LIST;
    }
    List<Fragment> newMenuFragments = new ArrayList<Fragment>();
    for (final Fragment fragment : fragments) {
      if (fragment != null && fragment.isAdded() && !fragment.mHidden && fragment.mHasMenu
          && fragment.mMenuVisible && fragment instanceof OnCreateOptionsMenuListener) {
        ((OnCreateOptionsMenuListener) fragment).onCreateOptionsMenu(menu, inflater);
        newMenuFragments.add(fragment);

        newMenuFragments.addAll(createChildrenMenus(menu, inflater, fragment
            .getChildFragmentManager().getFragments()));
      }
    }
    return newMenuFragments;
  }
  ...
  public boolean onCreatePanelMenu(int featureId, Menu menu) {
    ...
    if (featureId == Window.FEATURE_OPTIONS_PANEL) {
      ...
      boolean show = false;
      List<Fragment> newMenus = null;
      if (mFragments.mAdded != null) {
        newMenus = createChildrenMenus(menu, inflater, mFragments.mAdded);
        show = !newMenus.isEmpty();
      }
  ...
  private boolean prepareChildrenMenus(Menu menu, List<Fragment> fragments) {
    if (fragments == null || fragments.isEmpty()) {
      return false;
    }
    boolean show = false;
  ...
  public boolean onPreparePanel(int featureId, View view, Menu menu) {
    ...
    if (featureId == Window.FEATURE_OPTIONS_PANEL) {
      ...
      boolean show = false;
      if (mFragments.mAdded != null) {
        show |= prepareChildrenMenus(menu, mFragments.mAdded);
      }
  ...
  private boolean selectChildrenMenus(MenuItem item, List<Fragment> fragments) {
    if (fragments == null || fragments.isEmpty()) {
      return false;
    }
    boolean show = false;
    for (final Fragment fragment : fragments) {
      if (fragment != null && fragment.isAdded() && !fragment.mHidden && fragment.mHasMenu
          && fragment.mMenuVisible && fragment instanceof OnCreateOptionsMenuListener) {
        if (((OnOptionsItemSelectedListener) fragment).onOptionsItemSelected(item)) {
          show = true;
          continue;
        }

        selectChildrenMenus(item, fragment.getChildFragmentManager().getFragments());
      }
    }
    return show;
  }
  ...
  public boolean onMenuItemSelected(int featureId, MenuItem item) {
    ...
    if (featureId == Window.FEATURE_OPTIONS_PANEL) {
      ...
      if (mFragments.mAdded != null) {
        return selectChildrenMenus(item, mFragments.mAdded);
      }
    }
    return false;
  }
